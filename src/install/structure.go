...
func (hs *HandlerStruct) getConfig(cfg *src.Config, configDir string) (err error) {
	...
	hs.HostBaseDir = strings.ReplaceAll(src.SshCMDToGetOutput(fmt.Sprintf("ssh -o StrictHostKeyChecking=no %s@%s pwd", hs.Cfg.User, hs.Cfg.Masters[0])), "\n", "")
	return nil
}
...
func (hs *HandlerStruct) SCPK8sSetup(wg *sync.WaitGroup) (isOk bool) {
	...
	src.SshCMDToAllNodesByChannel(wg, hs.AllNodes,
		fmt.Sprintf("scp -o StrictHostKeyChecking=no %s %s", fmt.Sprintf("%s/k8s_setup.sh", hs.CfgDir),
...
func (hs *HandlerStruct) ExecuteK8sSetup(wg *sync.WaitGroup) (isOk bool) {
	stepIsOK := true
	src.SshCMDToAllNodesByChannel(wg, hs.AllNodes,
		fmt.Sprintf("ssh -o StrictHostKeyChecking=no %s sh %s", fmt.Sprintf("%s@nodeip", hs.Cfg.User),
...
func (hs *HandlerStruct) KubeadmInit() (kdmMasterJoinCMD string, kdmWorkerJoinCMD string) {
	kdmJoinCMD := fmt.Sprintf("ssh -o StrictHostKeyChecking=no %s sudo kubeadm init --kubernetes-version %s --pod-network-cidr %s --upload-certs",
...
func (hs *HandlerStruct) KubeadmJoin(wg *sync.WaitGroup, nodes []string, kdmJoinCMD string) bool {
	stepIsOK := true
	src.SshCMDToAllNodesByChannel(wg, nodes, fmt.Sprintf("ssh -o StrictHostKeyChecking=no %s sudo %s",
...
func (hs *HandlerStruct) ExecuteBashScript() {
	...
	filesCnt := len(files)
	err := exec.Command("scp", "-o", "StrictHostKeyChecking=no", "-r", deployDir,
	...
	for i := 0; i < filesCnt; i++ {
		err := exec.Command("ssh", "-o", "StrictHostKeyChecking=no", fmt.Sprintf("%s@%s", hs.Cfg.User, hs.Cfg.Masters[0]),
